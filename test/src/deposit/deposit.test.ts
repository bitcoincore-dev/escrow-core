...
export default async function (t : Test) {
  t.test('Deposit Tests', async t => {
    ...
    try {
      ...
      const alice_wallet = await client.get_wallet('test_alice')
      // const bob_wallet   = await client.get_wallet('test_bob')
      // const carol_wallet = await client.get_wallet('test_carol')

      const alice_recv   = await alice_wallet.newaddress

      if (VERBOSE) console.log('receive address:', alice_recv.address)

      // Create a deposit template that pays to Alice.
      const template = {
        vout : [{
          value : 800_000,
